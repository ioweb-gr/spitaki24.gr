...
class WordPress_GDPR_Services extends WordPress_GDPR
{
    ...
    public function meta_fields()
    {
        ...
        $html .= '<p class="post-attributes-label-wrapper">';
            ...
            $html .= '  <b>' . __('Is Google Adsense Service?', 'wordpress-gdpr') . '</b></label>';
        $html .= '</p>';


        // Adwords
        $checked = "";
        $adwords = get_post_meta($post->ID, 'adwords' , true);
        if($adwords == "1") {
            $checked = 'checked="checked"';
        }
        $html .= '<p class="post-attributes-label-wrapper">';
            $html .= '<label for="adwords"> <input ' . $checked . ' type="checkbox" value="1" name="adwords">';
            $html .= '  <b>' . __('Is Google Adwords / Remarketing Service?', 'wordpress-gdpr') . '</b></label>';
        $html .= '</p>';

        // Analytics
        $checked = "";
        $analytics = get_post_meta($post->ID, 'analytics' , true);
        if($analytics == "1") {
            $checked = 'checked="checked"';
        }
        $html .= '<p class="post-attributes-label-wrapper">';
            $html .= '<label for="analytics"> <input ' . $checked . ' type="checkbox" value="1" name="analytics">';
            $html .= '  <b>' . __('Is Google Analytics?', 'wordpress-gdpr') . '</b></label>';
        $html .= '</p>';
   ...
   /**
    ...
    public function save_custom_metaboxes($post_id, $post)
    {
        ...
        update_post_meta($post->ID, 'analytics', $_POST['analytics']);
        update_post_meta($post->ID, 'adwords', $_POST['adwords']);
