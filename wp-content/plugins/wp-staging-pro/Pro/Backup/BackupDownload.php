...
class BackupDownload
{
    public function listenDownload()
    {
        ...
        if (!isset($_GET['wpstgBackupDownloadMd5']) || !preg_match('/^[a-f0-9]{32}$/', $_GET['wpstgBackupDownloadMd5'])) {
            die('Invalid MD5.');
        }

        try {
            // Not using DI here since this runs on every request, so it can early bail without building dependencies.
            $backup = WPStaging::getInstance()->getContainer()->make(BackupsFinder::class)->findBackupByMd5Hash($_GET['wpstgBackupDownloadMd5']);
        } catch (\Exception $e) {
            die($e->getMessage());
        }
