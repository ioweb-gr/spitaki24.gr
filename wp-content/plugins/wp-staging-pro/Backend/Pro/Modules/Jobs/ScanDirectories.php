...
class ScanDirectories extends JobExecutable
{
    ...
    public function getStagingPlugins()
    {
        ...
        $excludeFolders = [
            ...
            '**/nbproject',
            $this->wpDirectories->getRelativePluginPath(SlashMode::BOTH_SLASHES) . 'wp-staging*',
            $this->wpDirectories->getRelativePluginPath(SlashMode::BOTH_SLASHES) . 'wps-hide-login',
        ];

        try {
            $iterator = new RecursiveDirectoryIterator($path, FilesystemIterator::SKIP_DOTS);
            $iterator = new RecursiveFilterExclude($iterator, apply_filters('wpstg_push_excl_folders_custom', $excludeFolders), $this->options->path);
    ...
    public function getStagingUploads()
    {
        ...
        $relpath = str_replace($this->options->path, '', $path);

        $excludeFolders = [
            '**/node_modules',
            '**/nbproject',
            $relpath . 'wp-staging',
            $relpath . 'cache', // can be removed
            $relpath . 'wps-hide-login', // can be removed
            $relpath . 'wp-content' // can be removed
        ];

        try {
            $iterator = new RecursiveDirectoryIterator($path, FilesystemIterator::SKIP_DOTS);
            $iterator = new RecursiveFilterExclude($iterator, $excludeFolders, $this->options->path);
            $iterator = new RecursiveIteratorIterator($iterator, RecursiveIteratorIterator::LEAVES_ONLY, RecursiveIteratorIterator::CATCH_GET_CHILD);
    ...
    public function getStagingUploadsOld()
    {
        ...
        $excludeFolders = [
            $this->wpDirectories->getRelativeUploadPath(SlashMode::BOTH_SLASHES) . 'wp-staging',
            '**/node_modules',
            '**/nbproject',
        ];
    ...
    private function getStagingUploadsNew()
    {
        ...
        $excludeFolders = [
            '/' . trailingslashit($this->getRelUploadDir()) . 'wp-staging',
            '**/node_modules',
            '**/nbproject',
        ];
