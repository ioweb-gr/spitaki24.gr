...
class Database extends JobExecutable
{
    ...
    /**
     * The db prefix of the production site
     * Should be always lowercase on Windows env
     * @var string
     */
    private $dbPrefix;
    ...
    public function initialize()
    {
        ...
        $this->dbPrefix             = WPStaging::getTablePrefix();
    ...
    private function getStagingPrefix()
    {
        ...
        if ($this->options->prefix === $this->dbPrefix) {
    ...
    private function stopExecution()
    {
        if ($this->tmpPrefix === $this->getStagingPrefix()) {
            $this->returnException('Fatal Error 9: Prefix ' . $this->getStagingPrefix() . ' is used for the staging site hence it can not be used as a temporary prefix. Please ask support@wp-staging.com how to resolve this.');
        }
